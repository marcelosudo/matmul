!pgf90 -acc -cuda -cudalib -Minfo=accel mainn2i.f90 -o mainn2i
...
program main
	...
	! print *, "a (max) = ", my
      ...
      print *,"mainn2i.f90 base 4", ni, " x ", nj, " ntimes = ", ntimes
      call cpu_time(t1)
	do nt = 1, ntimes
	!$acc kernels
      do j = 2, nj-1
         do i = 2, ni-1
			d(i,j) = alpha * (__half2float(a(i,j-1)) + __half2float(a(i,j+1)) + __half2float(a(i-1,j)) + __half2float(a(i+1,j)))
         end do
      end do
	 a = d
	!$acc end kernels
